...
export function SlideLibraryPanel({
  ...
  function handleReorder(stems: string[]) {
    ...
    for (const stem of stems) {
      ...
      if (group) {
        const ids = [group.files.slide?.id, group.files.style?.id, group.files.content?.id].filter(Boolean) as string[];
        fileIds.push(...ids);
      }
    }
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      <Button variant="accent" size="sm" className="w-full gap-1.5" onClick={onAddSlide}>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete "{deleteTarget?.title}"? This will remove the slide, style, and content files permanently.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
