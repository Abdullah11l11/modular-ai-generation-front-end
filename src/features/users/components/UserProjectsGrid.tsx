...
export function UserProjectsGrid({userId}:UserProjectsGridProps) {
    ...
    if(isLoading) return <div className='grid grid-cold-3 gap-4'>{Array.from({length:3}).map((_,i)=>
    <Skeleton key={i} className='h-40 rounded-xl'></Skeleton>)}</div>
        ...
        {(data?.data ?? MOCK_PROJECTS).map((project)=>(
            <Card key={project.id}>
                ...
                <CardContent>
                    ...
                    {project.origin_template_name && <p className='text-xs text-gray-500 mt-1'>from:{project.origin_template_name}</p>}
                </CardContent>
            </Card>
        ))}
    </div>)
